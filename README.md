...
### Accepting a "done" — `/saygoal:judge`, the acceptance gate

A claimed success needs the same scrutiny as a stall. `/goal`'s evaluator only pattern-matches output pasted into the transcript — nothing proves the paste was real — and a delegated model's "all tests pass" is a report, not evidence. `/saygoal:judge` closes that gap: it treats any completion report as **a set of claims**, re-runs the claimed verifications itself, diffs what actually changed, hunts the classic frauds in order of real-world frequency (assertions weakened to match broken behavior, expected values quietly rewritten, tolerances widened, real calls swapped for mocks, scope lies, leftover debris), and delivers an evidence-first verdict: **VERIFIED / VERIFIED WITH CAVEATS / REFUTED**. It reads and runs but never fixes, and it flags a conflict of interest when asked to judge its own session's work — fresh-context verification beats self-critique.

The test suite carries a trap for it: a fixture where the bug was never fixed, the frozen test was rewritten to assert the broken value, and the report claims "done, all tests pass, only the source file was touched". A live run returns REFUTED and names all four frauds — including that the report contradicts itself, since "updated the regression test" and "only touched the source file" cannot both be true. One run is a smoke test, not a rate (this repo's [`EXPERIMENT.md`](./EXPERIMENT.md) standard: treat anything under N≥10 as uncertain).

The design is adapted from [`Sahir619/fable-method`](https://github.com/Sahir619/fable-method)'s `fable-judge` (MIT) with one structural upgrade: fable's judge is generic, so it must reconstruct the claims from the report it is auditing — which lets the party under review define the exam. saygoal's judge is **contract-anchored, so the claims come free and were frozen before implementation started**: Verification says what to re-run, the verification surface says what to diff-audit, the write scope says where changes were allowed, the per-item deviation report is a ready-made claims table, and a compiled `saygoal.stop-check.sh` is a mechanical verdict core. Each verdict appends one line to `.claude/saygoal.history.jsonl`, so `/dec` grilling learns both how loops stall (from `/saygoal:retro`) and which frauds actually got caught. Deliberately manual — an acceptance button you press after a loop converges, a delegate reports back, or another session hands over work — never an automatic gate on every task.

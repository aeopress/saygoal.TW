...
- **`/saygoal:judge`** — 検収ゲート。「できました」を主張の集合として扱い、契約の検証を自分で走らせ直し、ものさし側ファイルの diff を監査して、**VERIFIED / VERIFIED WITH CAVEATS / REFUTED** の判定を返します。
...
それと、お願いがあいまいで契約の穴（達成ラインや検証先）が埋まらないときは、`/dec` がいきなり推測で埋めずに、**一度に一問ずつ**おすすめの答え付きで聞いてくれます。逆に十分はっきりしている依頼には何も聞かず、そのまま契約を書く——必要なときだけ確認する、という塩梅です（この挙動は Codex CLI で実機確認済み）。質問のすべてにおすすめの答えが付いているときは、その答えで組んだ**下書きの契約**を「要確認」印付きで一緒に見せてくれることもあります——答えがどんな契約になるかが先に見えるわけです。ただし、そのまま貼れる `/goal` の一行と委任は、あなたが確認するまで出しません。未確認の仮定を `/goal` に貼ると、それがそのまま要件になってしまうからです（Claude Fable 5.1 は放っておいてもこう振る舞うので、ルールは下書きの止めどころを固定しているだけです）。Claude Code チームの Thariq さんが語る Fable 5 の使い方——「最終的な spec を書く前に、実装について Claude に自分をインタビューさせる」（[元動画](https://x.com/trq212/status/2073100352921215386)）——とちょうど同じ流れです。
...
**「できました」の検収は `/saygoal:judge`**：成功の宣言には、行き詰まりと同じだけの疑いが要ります。`/goal` の評価役は transcript に貼られた出力をパターン照合するだけで、その出力が本当に実行されたものかは誰も証明していませんし、外注先の「全テスト通過」は報告であって証拠ではありません。`/saygoal:judge` は完成報告を**主張の集合**として扱い、宣言された検証を自分で走らせ直し、実際の変更を diff で確かめ、定番の不正（壊れた挙動に合わせて弱められたアサーション、こっそり書き換えられた期待値、スコープの嘘、残骸）を頻度順に狩って、**VERIFIED / VERIFIED WITH CAVEATS / REFUTED** を証拠つきで返します。テストスイートには罠が仕込んであります：バグは直っておらず、凍結したはずのテストが壊れた値を通すように書き換えられ、報告は「完了、全テスト通過、触ったのはソースだけ」と言う——実際に走らせると REFUTED が返り、4 つの不正が名指しされます（「回帰テストを更新した」と「ソースしか触っていない」は同時に真になり得ない、という報告自身の矛盾も含めて）。1 回の実行は煙テストであって合格率ではありません（本 repo の [`EXPERIMENT.md`](./EXPERIMENT.md) の基準：N≥10 未満は不確実として扱う）。元ネタは [`Sahir619/fable-method`](https://github.com/Sahir619/fable-method) の `fable-judge`（MIT）ですが、ひとつ構造的な強化があります：fable の判定役は汎用なので、審査対象の報告から主張リストを自分で組み立てるしかない——つまり**試験問題を受験者が作れてしまう**。saygoal の判定役は契約に錨を下ろしているので、主張リストは実装が始まる前に凍結済みのものがタダで手に入ります（Verification が「何を走らせ直すか」、ものさしパスが「どこを diff 監査するか」、書き込み範囲が「どこなら触ってよかったか」、項目別レポートがそのまま主張表、生成済みの stop-check が機械的な判定コア）。判定のたびに `.claude/saygoal.history.jsonl` に 1 行残り、次の `/dec` が「どんな不正が実際に捕まったか」まで学びます。あえて手動です——ループが収束したとき、外注が戻ってきたとき、別セッションの成果物を受け取ったときに、あなたが押す検収ボタン。

**外注するときは、契約に 3 つ足されます**：外注先とは会話ログを共有できませんし、途中で細かく直すのも難しい。なので委任チャネルを選ぶと `/dec` が 3 つ仕掛けを足します。ひとつめは **stop-check の生成**——投げる前に契約を `.claude/saygoal.stop-check.sh` に落とします。中では検証を自分で走らせ直し（貼られた出力は信じません）、ものさし側のファイル（テストやベンチのスクリプト）が投入時のコミットから動いていたら fail、各チェックの根拠を出力します。走り全体がぶら下がっているこの一箇所だけは、実装するモデル自身に判定させてはいけない——これで Claude と外注先が同じものさしを共有でき、「完了」の解釈が 2 つに割れません。ふたつめは **trace のファイル出力**——バックグラウンドの実行には読める transcript がないので、探索型の契約では 1 回の試行につき 1 行を `.claude/saygoal.trace.log` に追記します（時刻・実行者・試したこと → 結果 → 除外したこと）。`/saygoal:retro` は存在しない transcript を組み立て直す代わりに、このファイルを読みます。みっつめは **検収の入力を 3 つに絞ること**——契約の本文、diff、stop-check の出力と終了コード。それだけです（終了コードが 0 でなければ即不合格。実装の過程はあなたの context に戻しません）。

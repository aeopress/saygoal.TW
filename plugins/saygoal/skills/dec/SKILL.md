...
Boundaries: [allowed write paths / forbidden paths; verification-surface files that must not change; external systems read-only or draft-only].
...
- **Verify the verification first**: before emitting the contract, smoke-check each `Verification:` target read-only. If it doesn't exist yet, flag `⚠ verification does not exist yet — create it first` and make creating it the first step; otherwise the `/goal` loop fails on turn one. When creating the verification target is itself the task's deliverable (e.g. "write a script"), apply the same flag and make creating it step one — that is not a contradiction.
- **Name the verification surface**: list in `Boundaries:` the exact files the verification depends on (test files, bench scripts, CI config). They are the measuring instrument — touching them is cheating — and the explicit list lets a reviewer audit the final diff against it without replaying the process.
...
- **Search-type tasks get convergence guardrails**: when the task only converges through repeated try→verify cycles (performance tuning, flaky-test debugging, chasing a benchmark number), the classic stall is re-proposing the same change until the budget runs out — given the same state, the LLM falls back to its priors. Extend `Iteration policy:` with "keep a one-line search log per attempt: tried → result → ruled out", and add to `Constraints:` "do not repeat an approach whose verification output has already failed twice — each retry must differ in direction, not wording". Skip both for non-search tasks — there they are noise.
- **Loop cost scales with verification cost**: a `/goal` loop reruns verification every attempt. When the check is expensive (full e2e suite, long benchmark, complete build), tighten the iteration budget in `Stop when:` / `Pause if:`, or verify each attempt with a cheap targeted check (a single spec file) and run the full suite once at the end. On a tight token budget the contract also works without the loop — implement directly against it and use `Verification:` as the acceptance gate.
